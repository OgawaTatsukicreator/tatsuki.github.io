...
- **顔マスク処理アプリ**：ウェブフロンティア株式会社のインターンで取り組んだ画像処理アプリ。画像内の顔に絵文字スタンプを重ね、最大10枚を一括処理します。[ソースコード](https://github.com/OgawaTatsukicreator/face-mark-app)を参照できます。
- **KYOTABI＋**：関西ビギナーズハッカソンで制作した、京都のまだ知らないスポットを探す試作アプリ。公開デモには一部サンプルデータを使用しています。[公開デモ](https://kyoto-repeater.vercel.app/)と[ソースコード](https://github.com/OgawaTatsukicreator/kyoto_Beginners_hackathon_vol1)を参照できます。
...
作品の実装コードはこのリポジトリには含まれません。既存の個別作品ページには、本人が担当した範囲、設計上の判断、現在の状態を記載しています。ハッカソンとインターンの作品は公開コードで確認できる機能を紹介し、個別の担当範囲や非公開の業務内容は記載していません。
...
変更後はPCとスマートフォンの幅で、Homeの写真と「実績を見る」「自己紹介を見る」、5つの日本語見出しと「詳しく見る」、自己紹介→使用技術、実績→資格、ヘッダーの日英切替、モバイルメニュー、Achievementsの3タブと横スライドを確認してください。次のカードが見えるか、最後で案内が消えるか、スクロール時の文章表示が動きを減らす設定でも読めるかも確認します。既存作品から実績一覧へ戻れることを確認し、外部リンクや個人情報を含む画像を公開前に点検します。

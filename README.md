...
実績の個別ページとして `pages/work-face-mask.html`、`pages/work-kyotabi.html`、`pages/work-shift-system.html`、`pages/work-training-diary.html`、`pages/body-contest.html`、`pages/business-model.html` を使用します。`pages/work-coming-soon.html` は既存ファイルとして残していますが、現在の画面からはリンクしていません。

PCでは5つの主要項目をヘッダーに常時表示し、項目を押すと対応する詳細ページへ直接移動します。狭い画面では同じ項目をメニューにまとめます。Homeの5つの概要はカード全体が各詳細ページへのリンクで、小さな「詳しく見る」リンクは置きません。ヘッダーと概要カードは日本語を主表記、英語を補助表記とし、英語表示時は英語名だけに切り替わります。自己紹介の末尾から使用技術へ、実績の末尾から資格へ進めます。Achievementsの副メニューは、`#works`、`#experience`、`#internships` をURLに持つタブとして動作し、各分類のカードを横にスライドできます。

## 掲載している制作物

- **顔マスク処理アプリ**：ウェブフロンティア株式会社のインターンで扱った画像処理アプリ。現行コードには、最大10枚を選択し、顔の位置に絵文字スタンプを重ねる一括処理が実装されています。[詳細ページ](pages/work-face-mask.html)で仕様書と現行コードの差、確認範囲を説明しています。
- **KYOTABI＋**：関西ビギナーズハッカソンで制作した、京都のまだ知らない寄り道先を写真や条件から探すWebアプリ。[詳細ページ](pages/work-kyotabi.html)では公開版の機能と初期の要件定義書との差を示し、[公開デモ](https://kyoto-repeater.vercel.app/)にも進めます。
...
作品の実装コードはこのリポジトリには含まれません。個別作品ページでは、確認できる範囲で設計上の判断と現在の状態を記載しています。ハッカソンとインターンの作品は公開コードで確認できる機能を紹介し、個別の担当範囲や非公開の業務内容は記載していません。

## サイトの実装

新しいHomeと5つの詳細ページは `css/portfolio-v2.css` と `js/portfolio-v2.js` を使用します。日英切替は `js/portfolio-i18n.js`、実績タブの切替は `js/achievements.js` に分けています。既存の作品・経験ページは `css/style.css` を使い続け、共通ヘッダーを重ねています。
Skillsの技術・サービスアイコンは `assets/icons/` に保存し、[出典と利用条件](assets/icons/README.md)を記録しています。

画面の主な仕様は次のとおりです。

- 共通ヘッダーにはHomeへのロゴと5つの詳細ページへのリンクを配置し、現在のページを示します。日本語表示では日本語の項目名を大きく、英語を小さく添えます。英語表示では英語名だけにします。
- 新しい6ページではヘッダーのJP / ENから表示言語を切り替えられます。選択はブラウザ内に保存され、ページ移動後も引き継ぎます。既存の作品・経験の個別ページは本文が日本語のみのため切替ボタンを表示しません。英語を選択中の場合は、本文が日本語であることを画面上に示します。
- Homeの写真には既存の `images/back.png` を使っています。写真上に紹介文を置き、人物情報は写真の下に配置して、見出し・写真・次の行き先を見分けやすくしました。
- Homeの5つの概要カードは余白を含めた全体で押せます。カードごとにTabキーで1回フォーカスし、Enterキーで対応する詳細ページへ進めます。日英それぞれの言語で遷移先が分かる読み上げ名と、カード全体のフォーカス表示を備えます。
- 本文・説明文にはセリフ体、見出しとナビゲーション・ボタンにはサンセリフ体を使います。Home冒頭の「実績を見る」「自己紹介を見る」は同じ高さと角丸にそろえています。
- モバイルメニューはEscキーと背景のクリックでも閉じられ、開いている間は背後の本文へキーボードフォーカスが移りません。
- 実績のタブはキーボードの左右キー・Home・End・Enter・Spaceに対応し、ブラウザの戻る・進むでも選択を復元します。
- 実績の横スライドは、続きがある場合に次のカードの端と日英の案内を見せます。最後まで進むと案内が消え、2件とも収まるPC表示では操作部を表示しません。
- 旧ポートフォリオのスクロール時の文章表示を移し、画面に入った見出しや説明文だけを一度現します。リンクなどの操作要素は隠しません。JavaScript無効時と動きを減らす設定時は、文章を最初から表示します。
- すべてのページに本文へのスキップリンクを置き、キーボードフォーカスを表示します。動きを減らす設定では画面遷移の動きを抑えます。

色・余白・整列・ナビゲーションの見直しには、[UXPinのUIデザイン原則](https://www.uxpin.com/studio/jp/blog-jp/ui-design-principles-ja/)と[Picks DesignのUIデザインの考え方](https://picks-design.com/blog/5850/)を参考にしています。

## ローカル確認

静的サイトなので、リポジトリのルートをローカルサーバーで公開して `index.html` を開きます。VS CodeのLive Serverを使う場合、`.vscode/settings.json` のポートは `5501` です。

変更後はPCとスマートフォンの幅で、Homeの写真と「実績を見る」「自己紹介を見る」、5つの概要カード全体から対応ページへ進めること、カード内に「詳しく見る」が表示されないこと、自己紹介→使用技術、実績→資格、ヘッダーの日英切替、モバイルメニュー、Achievementsの3タブと横スライドを確認してください。HomeのカードはTab・Enter操作とカード全体のフォーカス表示、日英それぞれの読み上げ名も確認します。次のカードが見えるか、最後で案内が消えるか、スクロール時の文章表示が動きを減らす設定でも読めるかも確認します。4作品の詳細ページから実績一覧へ戻れることを確認し、外部リンクや個人情報を含む画像を公開前に点検します。

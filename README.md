...
PCでは5つの主要項目をヘッダーに常時表示し、項目を押すと対応する詳細ページへ直接移動します。狭い画面では同じ項目をメニューにまとめます。Homeの5つの概要は日本語見出しを先に表示します。自己紹介の末尾から使用技術へ、実績の末尾から資格へ進めます。Achievementsの副メニューは、`#works`、`#experience`、`#internships` をURLに持つタブとして動作し、各分類のカードを横にスライドできます。
...
変更後はPCとスマートフォンの幅で、Homeの写真と「実績を見る」「自己紹介を見る」、5つの日本語見出しと「詳しく見る」、自己紹介→使用技術、実績→資格、ヘッダーの日英切替、モバイルメニュー、Achievementsの3タブと横スライド、既存作品から実績一覧への戻りを確認してください。外部リンクは本人が指定した実URLだけを掲載し、個人情報を含む画像は公開前に点検します。

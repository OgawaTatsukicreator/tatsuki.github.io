...
PCでは5つの主要項目をヘッダーに常時表示し、項目を押すと対応する詳細ページへ直接移動します。狭い画面では同じ項目をメニューにまとめます。ヘッダーとHomeの5つの概要は日本語を主表記、英語を補助表記とし、英語表示時は英語名だけに切り替わります。自己紹介の末尾から使用技術へ、実績の末尾から資格へ進めます。Achievementsの副メニューは、`#works`、`#experience`、`#internships` をURLに持つタブとして動作し、各分類のカードを横にスライドできます。
...
変更後はPCとスマートフォンの幅で、Homeの写真と「実績を見る」「自己紹介を見る」、5つの日本語見出しと「詳しく見る」、自己紹介→使用技術、実績→資格、ヘッダーの日英切替、モバイルメニュー、Achievementsの3タブと横スライドを確認してください。次のカードが見えるか、最後で案内が消えるか、スクロール時の文章表示が動きを減らす設定でも読めるかも確認します。4作品の詳細ページから実績一覧へ戻れることを確認し、外部リンクや個人情報を含む画像を公開前に点検します。
